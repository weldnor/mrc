...
public class CallHandler extends TextWebSocketHandler {
    ...
    private MediaPipeline screenPipeline;
    private MediaPipeline webcamPipeline;
    private UserSession screenPresenterUserSession;
    private UserSession webcamPresenterUserSession;
        ...
        switch (jsonMessage.get("id").getAsString()) {
            case "screen-presenter":
                if (screenPresenterUserSession == null) {
                    screenPresenterUserSession = new UserSession(session);
            ...
            case "webcam-presenter":
                if (webcamPresenterUserSession == null) {
                    webcamPresenterUserSession = new UserSession(session);
            ...
            case "viewer":
                if (isEmpty(screenPresenterUserSession) || isEmpty(webcamPresenterUserSession)) {
                    ...
                    response.addProperty("message", "At least one sender no active now. "
                            + "Become sender or . Try again later ...");
    ...
    private boolean isEmpty(UserSession userSession) {
        return userSession == null || userSession.getWebRtcEndpoint() == null;
    }

    private void addCandidate(UserSession presenterUserSession, WebSocketSession session, JsonObject jsonMessage) {
    ...
    private synchronized void presenter(UserSession presenterUserSession, final WebSocketSession session, JsonObject jsonMessage, String type)
            throws IOException {
    ...
    private void alreadyActing(WebSocketSession session) {
        JsonObject response = new JsonObject();
    ...
    private synchronized void viewer(final WebSocketSession session, JsonObject jsonMessage, String type)
            throws IOException {
        UserSession viewer;
    ...
    private EventListener<IceCandidateFoundEvent> getListener(final WebSocketSession session, String iceCandidateType) {
        return event -> {
            JsonObject response = new JsonObject();
